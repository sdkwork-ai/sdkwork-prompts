...
```bash
npm install @sdkwork/prompts-sdk
# or
yarn add @sdkwork/prompts-sdk
# or
pnpm add @sdkwork/prompts-sdk
```

## Quick Start

```typescript
import { SdkworkPromptsOpenClient } from '@sdkwork/prompts-sdk';

const client = new SdkworkPromptsOpenClient({
  baseUrl: '/prompts/v3/api',
  timeout: 30000,
});
...
// Use the SDK
const result = await client.promptsPublic.prompts.catalog.list();
```
...
- `client.promptsPublic` - prompts_public API
...
```typescript
import { SdkworkPromptsOpenClient, NetworkError, TimeoutError, AuthenticationError } from '@sdkwork/prompts-sdk';
